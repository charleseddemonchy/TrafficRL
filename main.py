...
Author: [Your Name]
École Polytechnique - Master of AI
"""
...
def save_visualization(self, filename="traffic_simulation.mp4", fps=30, duration=30):
    """
    Save a video of the traffic simulation.
    ...
    try:
        ...
        # Color maps for traffic density
        ns_cmap = plt.cm.Blues
        ew_cmap = plt.cm.Oranges
        
        # Function to draw each frame
        def update(frame):
            if frame % 5 == 0:  # Take action every 5 frames
                # Use agent if available, otherwise use mix of random and smart actions
            ...
            for i in range(self.grid_size):
                for j in range(self.grid_size):
                    ...
                    # EW road
                    ew_road = plt.Rectangle(
                        (j, i + 0.45), 1, 0.1, 
                        facecolor='black', alpha=0.8
                    )
                    ax.add_patch(ew_road)
                    
                    # Draw traffic density as shaded areas along roads
                    # NS traffic density
                    ns_density = self.traffic_density[idx, 0]
                    ns_traffic = plt.Rectangle(
                        (j + 0.45, i), 0.1, 1, 
                        facecolor=ns_cmap(ns_density), alpha=0.7
                    )
                    ax.add_patch(ns_traffic)
                    
                    # EW traffic density
                    ew_density = self.traffic_density[idx, 1]
                    ew_traffic = plt.Rectangle(
                        (j, i + 0.45), 1, 0.1, 
                        facecolor=ew_cmap(ew_density), alpha=0.7
                    )
                    ax.add_patch(ew_traffic)
                    
                    # Draw intersection
                    intersection = plt.Rectangle(
                        (j + 0.45, i + 0.45), 0.1, 0.1, 
                    ...
                    # NS light (above intersection)
                    ns_light = plt.Circle((j + 0.5, i + 0.4), 0.03, color=ns_color)
                    ax.add_patch(ns_light)
                    
                    # EW light (left of intersection)
                    ew_light = plt.Circle((j + 0.4, i + 0.5), 0.03, color=ew_color)
                    ax.add_patch(ew_light)
                    
                    # Display traffic density values
                    ax.text(j + 0.2, i + 0.3, f"{ns_density:.2f}", fontsize=8, color='white', 
                             bbox=dict(facecolor='blue', alpha=0.5))
                    ax.text(j + 0.6, i + 0.7, f"{ew_density:.2f}", fontsize=8, color='white',
                             bbox=dict(facecolor='orange', alpha=0.5))
            
            # Title with time and overall statistics
            total_density = np.mean(self.traffic_density)
            ax.set_title(f"Traffic Simulation - Time: {frame/fps:.1f}s - Avg Density: {total_density:.2f}")
            
            # Legends
            ns_legend = plt.Rectangle((0, 0), 1, 1, facecolor=ns_cmap(0.7), alpha=0.7)
            ew_legend = plt.Rectangle((0, 0), 1, 1, facecolor=ew_cmap(0.7), alpha=0.7)
            ax.legend([ns_legend, ew_legend], ["NS Traffic", "EW Traffic"],
        ...
        if args.mode == 'train':
            ...
            try:
                ...
                env = TrafficSimulation(
                    ...
                    green_duration=CONFIG["green_duration"],
                    yellow_duration=CONFIG["yellow_duration"],
                    visualization=False,  # Don't need pygame visualization for video
                    random_seed=RANDOM_SEED
                )
