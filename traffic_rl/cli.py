...
from config import load_config, save_config, override_config_with_args
...
def benchmark_command(args, logger):
    ...
    for agent_type in agent_types:
        if agent_type == "random":
            agents_to_benchmark["Random"] = "random"
        elif agent_type == "fixed":
            agents_to_benchmark["FixedTiming"] = "fixed"
        elif agent_type == "adaptive":
            agents_to_benchmark["AdaptiveTiming"] = "adaptive"
        elif agent_type == "dqn" and args.model:
            if os.path.exists(args.model):
                agents_to_benchmark["TrainedDQN"] = args.model
            else:
                logger.error(f"Model file not found: {args.model}")
                continue
        else:
            logger.warning(f"Unknown or incompletely specified agent type: {agent_type}")
    
    if not agents_to_benchmark:
        logger.error("No valid agents to benchmark")
        return False
    
    logger.info("Benchmarking agents:")
    for agent_name, agent_info in agents_to_benchmark.items():
        logger.info(f"  {agent_name}: {agent_info}")
    ...
    for key, result in benchmark_results["results"].items():
        logger.info(f"  {key}:")
        logger.info(f"    Reward: {result.get('avg_reward', 'N/A'):.2f} ± {result.get('std_reward', 'N/A'):.2f}")
        logger.info(f"    Waiting Time: {result.get('avg_waiting_time', 'N/A'):.2f}")
        logger.info(f"    Throughput: {result.get('avg_throughput', 'N/A'):.2f}")
...
def analyze_command(args, logger):
    ...
    from analyze import run_comprehensive_analysis
...
def parse_args():
    ...
    bench_parser.add_argument("--agents", type=str, required=True,
                             help="Comma-separated list of agents to benchmark (random,fixed,adaptive,dqn)")
